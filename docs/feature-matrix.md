...
| NFS v4.0 | RFC 7530 | 🟡 | 🟡 | COMPOUND + SETCLIENTID/OPEN (UNCHECKED/GUARDED/EXCLUSIVE4)/CLOSE/OPEN_DOWNGRADE + byte-range LOCK/LOCKT/LOCKU + VERIFY/NVERIFY + lease expiry, reboot grace, READ/WRITE delegations + DELEGRETURN, **live CB_RECALL** (conflicting-open recall over the v4.0 back channel, timeout-revoke), SECINFO/LOOKUPP, and ACLs. |
| NFS v4.1 | RFC 8881 | 🟡 | 🟡 | EXCHANGE_ID, CREATE_SESSION, SEQUENCE (slot reply cache), DESTROY_SESSION/CLIENTID, RECLAIM_COMPLETE, pNFS files layout with **multi-DS striping**, and a session back channel (CB_SEQUENCE/CB_COMPOUND) that delivers CB_RECALL. |
...
| `RPCSEC_GSS` / Kerberos | 🟡 | RFC 2203 framing + handshake + pluggable GSS provider; a loopback fake mechanism exercises none/integrity/privacy in the offline suite. The **real Kerberos mechanism is implemented for Linux via `libgssapi_krb5`** (`[LibraryImport]`, AOT-safe) and verified by a CI job that stands up an MIT KDC + `nfs/...` keytab and runs a gated handshake + MIC + wrap/unwrap test. Windows SSPI remains gated/unimplemented. |
...
| pNFS (LAYOUT*, GETDEVICE*) | 🟡 | 🟡 | Files layout with **multi-DS striping**: GETDEVICEINFO returns multiple data-server addresses + a round-robin stripe-index map, LAYOUTGET returns a dense files layout with a configurable stripe unit, LAYOUTCOMMIT/RETURN. Exercised across ≥2 in-process DS endpoints over loopback. A first-class `Nfs4Client` pNFS fan-out API is not yet exposed (the layout/stripe map is followed explicitly). |
...
6. **Kerberos / RPCSEC_GSS** — the `.github/workflows/gss-kerberos.yml` workflow stands up an MIT KDC on `ubuntu-latest` (`eng/interop/krb5-setup.sh`: realm, client principal, `nfs/...` service keytab) and runs a gated integration test that performs a real `libgssapi_krb5` RPCSEC_GSS handshake plus MIC (integrity) and wrap/unwrap (privacy) round-trips. The offline suite uses a loopback mechanism and stays green without a KDC.
