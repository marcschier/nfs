...
| NFS v4.0 | RFC 7530 | 🟡 | 🟡 | COMPOUND + SETCLIENTID/OPEN/CLOSE + byte-range LOCK/LOCKT/LOCKU + lease expiry, reboot grace, READ delegations + DELEGRETURN, **live CB_RECALL** (conflicting-open recall over the v4.0 back channel, timeout-revoke), SECINFO/LOOKUPP, and ACLs. |
| NFS v4.1 | RFC 8881 | 🟡 | 🟡 | EXCHANGE_ID, CREATE_SESSION, SEQUENCE (slot reply cache), DESTROY_SESSION/CLIENTID, RECLAIM_COMPLETE, pNFS files layout (single DS), and a session back channel (CB_SEQUENCE/CB_COMPOUND) that delivers CB_RECALL. |
...
| `RPCSEC_GSS` / Kerberos | 🟡 | RFC 2203 framing + handshake + pluggable GSS provider; a loopback fake mechanism exercises none/integrity/privacy. The real krb5 (Linux libgssapi) / SSPI mechanism is provided but gated — it needs a KDC to verify. |
...
| pNFS (LAYOUT*, GETDEVICE*) | 🟡 | 🟡 | Files layout, single data server (MDS = DS): GETDEVICEINFO, LAYOUTGET/COMMIT/RETURN, loopback DS I/O. No striping or separate DS. |
