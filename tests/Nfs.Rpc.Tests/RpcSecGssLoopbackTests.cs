...
public sealed class RpcSecGssLoopbackTests
{
    [Fact]
    public async Task EstablishAsync_NullHandshake_CreatesContext()
    {
        ...
        RpcSecGssClientContext context = await RpcSecGssClientContext.EstablishAsync(
            ...
            version: 1,
            new LoopbackGssMechanism(),
            cancellationToken: TestContext.Current.CancellationToken);

        Assert.False(context.Handle.IsEmpty);
        Assert.Equal(128u, context.SequenceWindow);
    }

    [Theory]
    [InlineData(RpcSecGssService.None)]
    [InlineData(RpcSecGssService.Integrity)]
    [InlineData(RpcSecGssService.Privacy)]
    public async Task ProtectedDataCall_RoundTripsAndReachesProgram(RpcSecGssService service)
    {
        var program = new SecuredEchoProgram();
        await using var server = StartServer(program);
        await using RpcClient client = await RpcClient.ConnectAsync(
    ...
    {
        var security = new RpcSecGssServer(new LoopbackGssMechanism());
    ...
    {
        public const uint Number = 0x20000013;
